...
async def create_family_tree(
        tree_name: str = Form(...),
        mode: ModeEnum = Form(..., description="'self' for logged-in user, 'custom' for another user"),
    ...
    if mode == "self":
        ...
        ## note this is commented because user can create any number of trees.
    ...
    if profile_pic:
        ...
        profile_pic_path = os.path.join(UPLOAD_FOLDER, pic_full_name)
